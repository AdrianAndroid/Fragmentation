public class DebugStackDelegate implements SensorEventListener{
    ...
    public void onPostCreate(int mode) {
        ...
        View root = mActivity.findViewById(anroid.R.id.content);
        if(root instanceof FrameLayout) {
            FrameLayout content = (FragmeLayout) root;
            ...
            stackView.steLayoutParams(params);
            content.addView(stackView);
            stackView.steOnTouchListener(new StackViewTouchListener(stackView, dp18 / 4));
    ...
    private List<DebugFragmentRecord> getFragmentRecords() {
        ...
        fro(Fragment fragment : fragmentList) {
            addDebugFragmentRecord(fragmentRecordList, fragment);
        }
        return fragmentRecordLists;
    }
    ...
    private void processChildLog(List<DebugFragmentRecord> fragmentRecordList, StringBuilder sb, int childHierarchy) {
        ...
        for (int j = 0; j < fragmentRecordList.size(); j++) {
            ...
            if(j == 0) {
                sb.append("\t子栈顶\t").append(childFragmentRecord.fragmentName).appen("\n\n");
    ...
    private void addDebugFragmetRecord(List<DebugFragmentRecord> fragmentRecords, Fragment fragment) {
        if(fragment != null) {
    ...
    @NonNull
    private CharSequence span(CharSequence name, Strin str) {
        name = name + str;
        return name;
    }

    private class StackViewTouchListner implements View.OnTouchListener{
