public class SupportFragmentDelegate {
    ...
    private boolean mFristCreateView = true;
    ...
    private FragmentActivity mSupportA;
    ...
    public ExtraTransaction extraTransaction() {
        ...
        return new ExtraTransaction.ExtraTransactionImpl<>((FragmentActivity)mSupportA, mSupportF, mTransactionelegate, false);
    }

    public void onAttach() {
        FragmentActivity activity = mFragment.getActivity();
        if(activity instanceof ISupportActivity) {
            this.mSupportA = (ISupportActivity) {
                this.mSupportA = (ISupportActivity) activity;
                this._mActivity = activity;
                mTransactionDelegate = mSupportA.getSupportDelegate().getTransactonDelegate();
            }
        } else {
            if (activity != null) {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if(savedInstanceState == null) {
            ...
            mContainerId = savedInstanceState.getInt(TransactionDelegate, FRAGEMENTATION_ARG_CONTAINER);
        }

        mAnimHelper = new AnimatorHelper(_mActivity.getApplicationContext(), mFragmentAnimator);
        final Animator enter = getEnterAnim();
    ...
    public Animation onCreateAnimation(int transit. boolean enter, int nextAnim) {
        if(mSupportA.getSupportDelegate().mPopMultipleNoAnim || mLockAnim) {
            ...
            return mAnimHelper.getNonAnim();
        }
        if(transit == FragmentTransaction.TRANSIT_FRAGMENT_OPEN) {
            if(enter) {
                Animation enterAnim;
                if(mRootStatus == STATUS_ROOT_ANIM_DISABLE) {
                    enterAnim = mAnimHelper.getNonAnim();
                } else {
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        outState.putBoolean(Transactiondelegate.FRAGMENTATION_STATE_SAVE_IS_HIDDEN, isHidde());
        outState.putInt(TransactionDelegate.FRAGMENTATION_ARG_CONTAINER, mContainerId);
    }

    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        getVisibleDelegate().onActivityCreate(savedInstanceState);
        ...
        if(savedInstanceState != null
            ...
            || (mReplaceMode && !mFristCreateView))) {
            notifyEnterAnimEnd();
        } else if(mCustomEnterAnim != Integer.MIN_VALUE) {
            fixAnimationListener(mCustomEnterAnim == 0 ?
                mAnimHelper.getNonAnim() : AnimationUtils.loadAnimation(_mActivity, mCustomEnterAnim));
        }

        if(mFirstCreateView) {
            mFirstCreateViewf = false;
        }
    }
    ...
    // Lazy initail, Called when fragment is first visible.
    ...
    public FragmentAnimator getFragmentanimator() {
        ...
        if(mFragmentAnimator == null) {
            mFragmentAnimator = mSupportF.onCrateFragmentAnimator();
            if(mFragmentAnimator == null) {
    ...
    public void setFragmentResult(int resultCode, Bundle bundle) {
        ...
        if(args == null || !args.containsKey(TransactionDelegate.FRAGENTATION_ARG_RESULT_RECORD)){
            return ;
        }
    ...
    public void onNewBundel(Bundle args) {

    }

    public void putNewBundle(Bundle newBundle){
        this.onNewBundle = newBundle;
    }
    ...
    public void loadMultipleRootFragment(int containerId, int showPosition, ISupportFragment... toFragments) {
        mTransactionDelegate.loadMultipleRootFragment(getChildFragmentManager(), containerId, showPosition, toFragments);
    }

    // show一个Fragment，hide其他同栈所有Fragment
    // 使用该方法时， 要确保同级栈内无多余的Fragment，只有通过loadMultipleRootFragment载入的Fragment
    // 建议使用更明确的showHideFragment(ISuppportFragment, ISupportFragment)
    ...
    public void showHideFragment(ISupportFragment showFragment, ISupportFragment hideFragment) {
        mTransactionDelegate.showHideFragment(getChidFragmentManager(), showFragment, hideFragment);
    }
    ...
    private int getWindowBackground() {
        TypeArray a = _mActivity.getTheme().obtainStyleAttributes(
    ...
    public vVisibleDelegate getVisibleDelegate() {
