public class VisibleDelegate {
    ...
    // true = 曾经可见，也就是onLazyInitView执行过一次
    ...
    public void onSaveInstanceState(Bundle outState) {
        outState.putBoolean(FRAGMENTATION_STATE_SAVE_IS_INVISIBLE_WHEN_LEAVE);
    ...
    // public void onPause() {
        ...
        // 为了让下次resume时候，能正常的执行需要设置mAbortInitVisible, 来确保在resume的时候，可以执行完整的initVisible
    ...
    public void onHiddenChanged(boolean hidden) {
        if(!hidden && !mFragment.isResumed()) {
            // Activity 不是resume状态， 不用显示其下的fragment， 只需要设置标志位，待onResume时显示出来
            // if fragment is shown but not resumed, ignore...
            onFragmentShowWhenNotResumed();
            return;
        }
        if(hidden) {
            dispatchSupportVisible(false);
    ...
    private void onFragmentShownWhenNotResumed() {
        // fragment 需要显示，但是Activity状态不是resumed，下次resume的时候fragment需要显示，所以可以认为离开的时候可见
        mVisibleWhenLeave = true;
    
        mAbortInitVisible = true;
        dispatchChildOnFragmentShowWhenNotResumed();
    }

    private void dispatchChildOnFragmentShowWhenNotResumed() {
    ...
    private void safeDispatchUserVisibleHhint(boolean visible) {
        if(visible) {
            enqueueDispatchVisible();
        } else {
            if(mIsOnceVisible) {
    ...
    private void enqueueDispatchVisible() {
        mIdleDispatchSupportVisible = new MessageQueue.IdelHandler() {
            @Override
        ...
        Lopper.myQueue().addIdeHandler(mIdleDispatchSupportVisible);
    }
